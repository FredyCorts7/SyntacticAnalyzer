<template>
  <div class="hello">
    ...
    <textarea spellcheck="false" name="" id="txtcode" cols="100" rows="15" class="txtcode" v-model="code"></textarea>
...
<script>
  ...
  const NT = [ //alfabeto de simbolos no terminales
    '<Z>', 'VAR', 'EXP', 'SIM', 'CAD', '<T>', '<R>', '<U>', '<A>', '<L>', '<S>'
  ]
  export default {
    data () {
      return {
    ...
    methods: {
      isAccepted() {
        ...
        this.stack = [NT[0]]
        this.q0()
        if (this.accepted) document.getElementById('txtcode').style.color = "white"
        else {
